...
export class CombatService {
  ...
  addPlayer(socket: WebSocket, { id }: { id: string }) {
    ...
    this.broadcast({
      type: 'spawn',
      payload: { id, position: startPos, life: 100 }
    });
    this.broadcast({ type: 'system', payload: `${id} joined the battle` });
  ...
  removePlayer(socket: WebSocket) {
    ...
    if (player) {
      ...
      this.broadcast({ type: 'system', payload: `${player.id} left the battle. Game over.` });
    }
  }
  ...
  handleAction(socket: WebSocket, action: any) {
    ...
    if (action.kind === 'move') {
      ...
      this.currentTurnIndex = (this.currentTurnIndex + 1) % this.sessions.length;
      ...
      this.broadcast({ type: 'action', payload: { kind: 'pass', id: action.id } });
      this.notifyTurn();
      ...
      if (distance > def.range || distance === 0) {
        socket.send(JSON.stringify({ type: 'error', payload: 'Target out of range' }));
        return;
      }
      const res = this.performAttack.execute({ attackerId: id, targetId: target, attackType });
      if (!res.success) {
